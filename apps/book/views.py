...
def book_list(request, page=0, **kwargs):
...
def book_detail(request, slug, **kwargs):
...
def search(request, template_name='book/book_search.html'):
    context = {}
    if request.GET:
        ...
        if len(cleaned_search_term) != 0:
            book_list = Book.objects.published().filter(Q(body__icontains=cleaned_search_term) | Q(tags__icontains=cleaned_search_term) | Q(categories__title__icontains=cleaned_search_term))
            context = {'object_list': book_list, 'search_term':search_term}
        else:
